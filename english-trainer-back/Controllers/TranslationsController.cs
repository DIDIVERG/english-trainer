...
namespace english_trainer_back.Controllers
{
    ...
    [ApiController]
    public class TranslationsController : BaseCrudController<Translations,TranslationsController>
    {
        public TranslationsController(IMapper mapper,ITranslationRepo baseRepo, ILogger<TranslationsController> logger): 
            base(mapper,baseRepo, logger)
        {
        }
        [HttpPut("Update/{id}")]
        public async Task<ActionResult<Languages>> UpdateOne(int id , TranslationsDto entity)
        {
            ...
            var entityToUpdate = mapper.Map<Translations>(entity);
